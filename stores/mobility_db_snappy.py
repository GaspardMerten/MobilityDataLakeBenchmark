...
class MobilityDBBatchCompressedSnappyStore(BaseStore):
    ...
    def store_buffer(self):
        ...
        for uuid, data in features_data.items():
            ...
            self.connection.execute(
                ...
                VALUES (
                    '{result[0]}',
                    E'{snappy.compress(result[1].encode()).hex()}',
                    E'{snappy.compress(result[2].encode()).hex()}',
                    E'{snappy.compress(result[3].encode()).hex()}',
                    E'{snappy.compress(result[4].encode()).hex()}',
                    E'{snappy.compress(result[5].encode()).hex()}',
                    E'{snappy.compress(result[6].encode()).hex()}',
                    E'{snappy.compress(result[7].encode()).hex()}',
                    E'{snappy.compress(result[8].encode()).hex()}',
                    E'{snappy.compress(result[9].encode()).hex()}'
                )
            """
                )
            )
